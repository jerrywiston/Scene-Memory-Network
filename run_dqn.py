...
def train(env, agent, stack_frames, img_size, exp_path="experiments_rl", eps_steps=1000, max_steps=1000000):
    ...
    while True:
        ...
        state = np.concatenate([info["render"].transpose(2,0,1)/255., state], 0)
        state_dict = {"obs":state}
        if GET_CELL:
            state_dict = {"obs":state, "cell_s2":info["cell_s2"], "code_s2":info["code_s2"]}
        ...
        while True:
            ...
            state_next = np.concatenate([state_next, state[2*3:]], 0)
            state_next = np.concatenate([info["render"].transpose(2,0,1)/255., state_next], 0)
            state_next_dict = {"obs":state_next}
            if GET_CELL:
                state_next_dict = {"obs":state_next, "cell_s2":info["cell_s2"], "code_s2":info["code_s2"]}
...
if __name__ == "__main__":
    ...
    maze_obj = maze.MazeGridRandom2(size=(17,17), room_total=15)
    env = maze_rep_env.MazeSceneRepEnv(maze_obj, render_res=(64,64), exp_path='experiments/model_smn/', n_items=n_items)
    stack_frames = 5
    img_size = (64,64)
    ...
    agent = dqn.DQNAgent(
        n_actions = 5,
        input_shape = [(stack_frames+1)*3, *img_size],
        #qnet = models.QNetTSM,
        qnet = models.QNetCellTSM,
        device = device,
        learning_rate = 2e-4, 
        reward_decay = 0.95,
